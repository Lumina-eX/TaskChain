...
vi.mock('@/lib/security/fileEncryption', () => ({
  ...
  storeEncryptedFile: vi.fn().mockResolvedValue({ iv: 'dummy-iv', filePath: '/tmp/evidence-file' }),
...
describe('GET /api/disputes/[id]/evidence', () => {
  it('returns 403 when the user is not a dispute participant or admin', async () => {
    ...
    const response = await GET(makeGetRequest('http://localhost/api/disputes/42/evidence'), makeContext())
...
describe('POST /api/disputes/[id]/evidence', () => {
  ...
  it('stores encrypted evidence metadata for authorized users', async () => {
    queueSql([
      ...
      [{ id: 9, file_name: 'proof.pdf', file_hash: 'dummy-hash', created_at: '2026-01-01T00:00:00Z' }],
      [],
    ])
